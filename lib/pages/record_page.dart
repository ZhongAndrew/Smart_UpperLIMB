...
class _RecordPageState extends State<RecordPage> {
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                Text(
                  _formattedTime,
                  style: const TextStyle(fontSize: 64, fontWeight: FontWeight.bold, fontFamily: 'monospace', letterSpacing: 2, color: Color(0xFF1E293B)),
                ),
                const SizedBox(height: 24),
      ...
      case RecordState.initial:
        return SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: const Text('校正基準', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ),
        );
      ...
      case RecordState.recording:
        return SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: const Text('停止錄製', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ),
        );
        ...
        return Row(
          ...
          children: [
            OutlinedButton.icon(
              ...
              label: const Text('刪除', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: ElevatedButton.icon(
                ...
                label: const Text('匯出 CSV', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
              ),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: ElevatedButton.icon(
                ...
                label: const Text('分析', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        );
            ...
            children: [
              ...
              if (!isConnected)
                Container(
                  ...
                  child: Text('未同步', style: TextStyle(fontSize: 12, color: Colors.red.shade400, fontWeight: FontWeight.bold)),
                ),
            ],
          ),
          const Padding(
  ...
  // 💡 根據 isConnected 決定要顯示動態波形圖，還是灰色的「尚未連線並同步」佔位框
    ...
    return Column(
      ...
      children: [
        ...
        Expanded(
          child: isConnected
              ? _LiveWaveChart(
            isRunning: _currentState != RecordState.completed,
            isRecording: _currentState == RecordState.recording,
            lineColor: color,
            lineCount: lines,
          )
              : Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text('尚未連線並同步', style: TextStyle(color: Colors.grey.shade400, fontWeight: FontWeight.bold, fontSize: 13)),
              ],
            ),
          ),
        ),
...
class _LiveWaveChart extends StatefulWidget {
  ...
  @override
  State<_LiveWaveChart> createState() => _LiveWaveChartState();
}
...
class _WavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < lineCount; i++) {
      ...
      final linePaint = Paint()
        ..color = color.withValues(alpha: i == 0 ? 0.8 : (1.0 - (i * 0.3)).clamp(0.2, 0.6))
  ...
  bool shouldRepaint(covariant _WavePainter oldDelegate) {
    return oldDelegate.phase != phase || oldDelegate.isRecording != isRecording;
  }
}