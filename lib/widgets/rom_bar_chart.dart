...
class RomComparisonChart extends StatelessWidget {
  ...
  // ============================================================================
  // 上半部：橫向視覺化長條圖 (Horizontal Bar Chart)
  // ============================================================================
  ...
  Widget _buildHorizontalBarRow(String side, double value, Color color) {
    double fraction = (value / 180.0).clamp(0.0, 1.0); // 最大180度

    return Row(
      children: [
        SizedBox(
          width: 24,
          child: Text(side, style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.bold, fontSize: 13)),
        ),
        Expanded(
          child: Stack(
            children: [
        ...
        SizedBox(
          width: 36,
          child: Text(value > 0 ? '${value.round()}°' : '--',
              textAlign: TextAlign.right,
              style: TextStyle(color: value > 0 ? color : Colors.grey, fontWeight: FontWeight.bold, fontSize: 13)
          ),
        ),
      ],
    );
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Text('$index. $exerciseName', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
              ],
            ),
          ),
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            ...
            child: Text('共 $count 次', style: TextStyle(fontSize: 11, color: Colors.grey.shade600, fontWeight: FontWeight.bold)),
          )
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(
              width: 60,
              child: Text('第 $repNumber 次', style: TextStyle(fontSize: 14, color: Colors.grey.shade700, fontWeight: FontWeight.bold))
          ),

          if (isComplex && data.dir != null && data.dir!.isNotEmpty)
            SizedBox(
                width: 50,
                child: Text('(${data.dir})', style: const TextStyle(fontSize: 12, color: Color(0xFF3B82F6), fontWeight: FontWeight.bold))
            ),
          ...
          SizedBox(
            width: 70,
            child: Text('幅度 ${romVal}°', textAlign: TextAlign.right, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: themeColor)),
          ),
        ],
      ),
    );
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: const Center(
        child: Text('⚠️ 目前無動作數據可供分析', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
      ),
    );
  }
}